...
root_dir,DEVICE,is_colab = set_root_dir() #set the root and the device used
sys.path.append('src/dbn_utls')
...
def data_and_labels(data_train: Dataset, BATCH_SIZE: int,
    ...
    train_loader = DataLoader(data_train, batch_size=BATCH_SIZE, shuffle=False, generator=torch.Generator(device=DEVICE)) #create a dataloader with the data shuffled
...
def tool_loader_ZAMBRA(selected_idx = [], only_data = False,classifier_yn = True, last_layer_sz = 1000, Load_DBN_yn = 3):
    ...
    with open(path.join(root_dir, f'lparams-{DATASET_ID.lower()}.json'), 'r') as filestream:
        LPAR = json.load(filestream)
    filestream.close()
    
    train_dataset_original, test_dataset_original = load_data_ZAMBRA(DATASET_ID,LPAR['BATCH_SIZE'],root_dir) #load the dataset of interest
    ...
    if Load_DBN_yn == 0: #if the user chose to train a iDBN from scratch...
        ...
        for run in range(CPAR['RUNS']):
            ...
            torch.save(dbn.to(torch.device('cpu')),
                        open(path.join(root_dir, f'{fname}.pkl'), 'wb'))
        ...
        dbn = torch.load(path.join(root_dir, fname+'.pkl'))
        if not(hasattr(dbn, 'fname')):
            dbn.fname = fname
            torch.save(dbn.to(torch.device('cpu')), open(path.join(root_dir, f'{fname}.pkl'), 'wb'))
...
def classifier_loader(dbn,train_dataset_original, test_dataset_original, selected_idx = [], DEVICE = 'cuda'):
    if dbn.dataset_id == 'MNIST':
        ...
        PATH = os.path.join(root_dir,'VGG16_MNIST','VGG16_MNIST_best_val.pth')
        ...
        if Load_classifier ==0: #if i want to train the classifier from scratch...
            ...
            train_dataloader = Multiclass_dataset(train_dataset_original, selected_idx = selected_idx, for_classifier = True, Old_rbm=False, DEVICE =DEVICE)
            test_dataloader = Multiclass_dataset(test_dataset_original, selected_idx = selected_idx, for_classifier = True, Old_rbm=False, DEVICE =DEVICE)
